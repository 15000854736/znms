...
function refreshStartMap(url,maxValue,point, radius,scale,pointSize,myChart){
	var time1 = new Date();   
	//console.log("*********"+time1);
	$.ajax({
		type:'POST',
		...
		success:function(xydata){
			...
			for(var i=0;i<xydata.length;i++){
				...
				var random_data= getPoint(radius,x,y, point*xydata[i].value);
...
function createStarMap(url, maxValue, point, radius, scale, pointSize) {
	var myChart = echarts.init(document.getElementById("star_map"));
	...
	$.ajax({
		...
		success:function(data){
			//console.log(data);
			drawStarMap(myChart,data,point,radius,scale,pointSize);
		}
	});
...
function drawStarMap(myChart, xydata, point, radius, scale, pointSize){
	var dom = $("#star_map");
	var width = dom.width();
	var height = dom.height();
