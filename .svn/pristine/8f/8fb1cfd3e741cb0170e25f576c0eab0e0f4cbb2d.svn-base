...
function selectChange(){
	var id=$("#dummyHost").val();
	$.ajax({
			 type:"post",
			 url:"${pageContext.request.contextPath}/backupConfiguration/getBackRoute",
			 data:{'hostUuid':id},
			 dataType:'json',
			 success:function(data){
			 	 //console.log(data.backRoute);
				 $("#content").val(data.backRoute);
			 }
		  });
}	
</script>
				...
				<form:form id="deviceForm" action="${pageContext.request.contextPath }/backupConfiguration/mergeDevice" method="post" modelAttribute="device">
					...
					<table border="0" cellspacing="0" cellpadding="0" class="nms_data_tablemod padding_10">
						...
						<tr>
							...
							<td>
								<div id="dummyHost" onchange="selectChange()"></div>
