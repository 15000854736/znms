...
function reflush(){
	$.ajax({
		url:"${pageContext.request.contextPath}/hostListbox/reflush",
		type:"post",
		dataType:"json",
			...
			for(var i=0;i<data.length;i++){
				...
				if(bean.hostList.length==0){
					...
					for(var j=0;j<bean.hostList.length;j++){
						...
						row += "<dd>延时："+host.curTime+"毫秒</dd><dd style='width:250px'>最后宕机时间："+
						host.lastShutDownTime+"</dd><dd>可用性: "+host.availability+"%</dd></dl>";
				...
				<c:choose>
					...
					<c:otherwise>
						<c:forEach items="${bean.hostList }" var="host">
							...
							<dd>延时：${host.curTime }毫秒</dd>
							<dd style="width:250px">最后宕机时间：<fmt:formatDate value="${host.lastShutDownTime }" type="date" pattern="yyyy-MM-dd  HH:mm:ss"/></dd>
							<dd>可用性：${host.availability}%</dd>
							</dl>
						</c:forEach>
					</c:otherwise>
				</c:choose>
