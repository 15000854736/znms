...
<script type="text/javascript">	
	...
	function hideFullScreen(){
		...
		$("#full_screen_leida").hide();
		$("#full_screen_heatbox").hide();
	...
	function showLeidaFullScreen(){
		...
		$("#screen_css_base").attr("href", "<%=path%>/css/znms_style_show_base.css");
		$("#main_css").attr("href", "<%=path%>/css/blank.css");
		$("#full_screen_leida").siblings("div").hide();
	...
	function getStarMapBgScale(){
		...
		var widthScale = starMapWidth/screenWidth;
		var heightScale = starMapHeight/screenHeight;
	...
	function showHeatBoxFullScreen(){
		...
		$("#star_map").height(scale*${starMapBgHeight});
		$("#star_map").width(scale*${starMapBgWidth});

		var star_map = createStarMap("<%=path%>/screen/getCenterUpAreaData",apMaxUserCount,point,radius,scale,pointSize);
		
		refresher1 = setInterval(function(){
			refreshStartMap("<%=path%>/screen/getCenterUpAreaData",apMaxUserCount,point,radius,scale,pointSize,star_map);
		}, 5000);
...
<body>
	<div>
		...
		<div class="screen_grid">
			...
			<div>
				<div>
					<div class="R" onclick="javascript:showLeidaFullScreen();">
						<a href="javascript:void(0);" title="推荐分辨率:&#10;1920 &#215; 1024&#10;1600 &#215; 900&#10;1366 &#215; 768">
							<img src="<%=path%>/images/fullimg.png"/>
						</a>
					</div>
	...
	<div id="full_screen_leida">
		...
		<div class="prj_top1">
			<c:choose>
				<c:when test="${rightUp < 60}">
					<div class="network_health1" style="color:red">
						<h1 id="network_health_title1">网络状态不良</h1>
						<span class="led_fonts" id="network_health_value">${rightUp}</span>
						<div class="network_health_imgbg"></div>
					</div>
				</c:when>
				<c:when test="${rightUp < 80}">
					<div class="network_health1" style="color:yellow"> 
						<h1 id="network_health_title">网络状态正常</h1>
						<span class="led_fonts" id="network_health_value">${rightUp}</span>
						<div class="network_health_imgbg1"></div>
					</div>
				</c:when>
				<c:otherwise>
					<div class="network_health1">
						<h1 id="network_health_title1">网络状态健康</h1>
						<span class="led_fonts" id="network_health_value">${rightUp}</span>
						<div class="network_health_imgbg1"></div>
					</div>
				</c:otherwise>
		...
		<div id="full_screen_heatbox" style="display:none;" >
			<div class="prj_logo">
				<img src="<%=path%>/image/${logoImage}"/>
			</div>
			<div class="diagram_box1">
				<div id="star_map" style="width:1506px; height: 1078px; background-repeat: no-repeat" >
				</div>
			</div>
		</div>
</body>
