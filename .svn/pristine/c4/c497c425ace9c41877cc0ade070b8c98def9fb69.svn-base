...
function checkMac(value){
	...
	var exp_ = /[A-Fa-f0-9]{2}-[A-Fa-f0-9]{2}-[A-Fa-f0-9]{2}-[A-Fa-f0-9]{2}-[A-Fa-f0-9]{2}-[A-Fa-f0-9]{2}/;
    if(exp.test(value)||exp_.test(value)){
    	return true;
    }
    return false;
}
...
function checkMobile(value){
	var exp = /^(0|86|17951)?(13[0-9]|15[012356789]|17[0678]|18[0-9]|14[57])[0-9]{8}$/;
	if(exp.test(value)){
    	return true;
    }
	return false;
...
function validFormOnSubmitPrefab(){
	...
	$("input[type=text]:not(:disabled):not(.Wdate):not(.unfilter):not(.unvalidate)").each(function(){
		checkChar(this);
    });
