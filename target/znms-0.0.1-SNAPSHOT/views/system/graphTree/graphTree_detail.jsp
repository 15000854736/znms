...
<style type="text/css">
 .but_save_custom{ padding: 0 10px; background:#09d; color:#fff; margin:0 0 0 5px;  height: 20px;} 
</style>
		...
		row += '<td class="text_left " width="70%">';
			row += '<div style=" text-indent:'+nodeFloor+'em;">';
				...
				row += '<span class="text_left"> ';
					...
					row += '<button type="button" class="but_save_custom" role="save" ';
					if(graphTreeItemUuid !=''){
						row += 'style="display:none"';
					}
					row += '>保存</button>';
					row += '<button type="button" class="but_save_custom" role="cancel" style="display:none">取消</button>';
				row += '</span>';
			row += '</div>';
		row += '</td>';
		row += '<td>';
				...
				<form id="graphTreeItemForm">
					<table border="0" cellspacing="0" cellpadding="0" class="nms_data_tablemod padding_10">
						...
						<tr id="titleTr">
							...
							<td>
							<input id="title" name="title" type="text" style="width:70%"  maxlength="255" class="form-control input-sm" />
